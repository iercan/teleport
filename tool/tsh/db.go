...
// dbInfoHasChanged checks if cf.DatabaseUser username matches the username in the database certificate.
func dbInfoHasChanged(cf *CLIConf, certPath string) (bool, error) {
	...
	if cf.DatabaseUser != "" && cf.DatabaseUser != identity.RouteToDatabase.Username {
		fmt.Printf("Will reissue database certificate for user %q (was %q)\n", cf.DatabaseUser, identity.RouteToDatabase.Username)
		return true, nil
	}
	if cf.DatabaseName != "" && cf.DatabaseName != identity.RouteToDatabase.Database {
		fmt.Printf("Will reissue database certificate for database name %q (was %q)\n", cf.DatabaseName, identity.RouteToDatabase.Database)
		return true, nil
	}
	return false, nil
}
